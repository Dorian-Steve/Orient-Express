            ...
            >
              {formatter && item.value !== undefined && item.name ? (
                formatter(
                  ...
                  item,
                  index,
                  payload.filter((p): p is TooltipPayloadItem => p !== undefined),
                )
              ) : (
                <>
                  {itemConfig?.icon ? (
...
function getPayloadConfigFromPayload(
  ...
  // Check payload.payload if it exists
  else if (
    "payload" in payloadRecord &&
    typeof payloadRecord.payload === "object" &&
    payloadRecord.payload !== null &&
    key in payloadRecord.payload &&
    typeof (payloadRecord.payload as Record<string, unknown>)[key] === "string"
  ) {
    configLabelKey = (payloadRecord.payload as Record<string, string>)[key];
  }

  if (configLabelKey === undefined) {
    return config[key];
  }
